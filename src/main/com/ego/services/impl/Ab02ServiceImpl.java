...
public class Ab02ServiceImpl extends JdbcServicesSupport
{
	...
	/**
	 * 通过关键字搜索商品, 模糊匹配商品名称,根据 id进行分页查询  每页13个商品
	 * hug
	 * @return
	 */
	private List<Map<String,String>> queryByKey() throws Exception
	{
		String sql="select aab202,aab203,aab205,aab206,aab208 from ab02 where aab212='02' and aab202 like ? limit ?,13";
	 ...
	 * 查询匹配某个关键字的商品总个数
	 */
	private Map<String,String> findProCountByKey() throws Exception
	{
		String sql="select count(aab202) prosum from ab02 where aab212='02' and aab202 like ?";
		return this.queryForMap(sql, this.get("key"));
	}
	
	/**
	 * 通过分类进行搜索,类别可能是第一级类别,第二级类别,第三季类别,多表联合擦查询  分页查询  每页13个
	 * hug
	 * @return
	 */
	 ...
	 * 查询匹配某个商品类型的商品总个数
	 */
	private Map<String,String> findProCountBySort() throws Exception
	{

		StringBuilder sql=new StringBuilder()
				.append("select count(x.aab202) prosum")
				.append("  from ab02 x, syscode s, syscode y,syscode z")
				.append(" where ((x.aab204=?       and s.fcode =?       and y.fcode=s.pfcode and z.fcode=y.pfcode)")
				.append("    or  (x.aab204=s.fcode and s.pfcode=?       and y.fcode=?        and z.fcode=y.pfcode)")
				.append("    or  (x.aab204=s.fcode and s.pfcode=y.fcode and y.pfcode=?       and z.fcode=?))")
