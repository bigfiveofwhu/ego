...
<div class="container mt-4 ml-4">
	<%--aab501(流水) ,aab502(卡券类型),aab503(优惠值),aab504(条件),aab505(持续时间),aab506(数量) --%>
		<c:choose>
			...
			<c:otherwise>
				<c:forEach  items="${userCoupons}" var="item">
					<c:choose>
						<c:when test="${item.aaa503<20.0 }">
							<div class="stamp stamp01 w-auto float-left mr-4" id="${item.aaa501}">
						</c:when>
						<c:when test="${item.aaa503<50.0 }">
							<div class="stamp stamp02 w-auto float-left  mr-4" id="${item.aaa501}">
						</c:when>
						<c:when test="${item.aaa503<100.0 }">
							<div class="stamp stamp03 w-auto float-left mr-4" id="${item.aaa501}">
						</c:when>
						<c:otherwise>
							<div class="stamp stamp04 w-auto float-left mr-4" id="${item.aaa501}">
						</c:otherwise>
					</c:choose>
					            <div class=" par  w-auto">
						            ...
						            <sub>优惠券</sub>
					            	<c:if test="${item.aaa502=='2'}"><p >订单满${item.aaa504}元</p></c:if>
					            </div>
					            <div class="copy w-auto mt-4">有效期至
					            	<p>${item.aaa506}</p>
					            </div>
			       			 </div><!-- stamp结束位置 -->
			     </c:forEach>
